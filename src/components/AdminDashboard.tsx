...
import {
  ...
  Image as ImageIcon, MapPin, Calendar, RefreshCw,
  Activity, BarChart2 as BarChartIcon
} from 'lucide-react';
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${map[status] || 'bg-slate-100 text-slate-600'}`}>
      {status.replace(/_/g, ' ')}
    </span>
  );
};
...
// ─── Issue Table Row ───────────────────────────────────────────────────────────
const IssueRow: React.FC<{ issue: any; index: number }> = ({ issue, index }) => (
  <motion.tr
  ...
  >
    {/* Thumbnail */}
    <td className="py-4 pl-4 pr-3 w-16">
      {issue.photo_url ? (
        <div className="w-12 h-12 rounded-xl overflow-hidden border border-slate-200 shrink-0">
          <img src={issue.photo_url} alt={issue.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
        </div>
      ) : (
    ...
    <td className="py-4 pr-4">
      <p className="font-semibold text-slate-900 text-sm line-clamp-1">{issue.title}</p>
    ...
    <td className="py-4 pr-4 hidden md:table-cell">
      <div className="flex items-center gap-1.5 text-slate-500 text-sm">
        ...
        <span className="truncate max-w-[140px]">{issue.locality}</span>
      </div>
    </td>
    ...
    <td className="py-4 pr-4 hidden lg:table-cell">
      <div className="flex items-center gap-1.5 text-slate-400 text-xs">
        ...
        {issue.createdAt || issue.created_at
          ? formatDistanceToNow(new Date(issue.createdAt || issue.created_at), { addSuffix: true })
          : '—'}
      </div>
    </td>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="py-3 pl-4 pr-3 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest w-16">Photo</th>
                    <th className="py-3 pr-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest">Title / Category</th>
                    <th className="py-3 pr-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden md:table-cell">Location</th>
                    <th className="py-3 pr-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden lg:table-cell">Reported</th>
                    <th className="py-3 pr-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {issues.map((issue, i) => (
                    <IssueRow key={issue.id || issue._id} issue={issue} index={i} />
                  ))}
                </tbody>
              </table>
            </div>
